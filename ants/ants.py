...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            #if we want to add an ant inside a container ant thats already been placed
            elif self.ant.can_contain(insect):
                self.ant.contain_ant(insect)

            #if we want to envelope a placed ant with a container ant
            elif insect.can_contain(self.ant):
                insect.contain_ant(self.ant)
                self.ant = insect

            #want to place two non bodygaurd ants
            else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if hasattr(insect, 'name') and insect.name == 'Queen':
                if insect.impostor == False:
                    return
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    armor = 1
    min_range, max_range = 0, 100000000  # arbitrary large number
    ...
    def nearest_bee(self, hive):
        ...
        space = self.place  #heavily modded
        while space is not hive and space is not None:
            if total >= self.min_range and total <= self.max_range and space.bees != []:
                return random_or_none(space.bees)
            total += 1
...
# The WallAnt class
class WallAnt(Ant):
    """wall ant doesnt do anything it just tanks bees"""
    name = 'Wall'
    food_cost = 4
    implemented = True

    def __init__(self, armor = 4):  #changed after testing
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.contained_ant == None:
            return None
        self.contained_ant.action(colony) #colony?
        # END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        super(Water, self).add_insect(insect)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.place is not None:
            myPlace = self.place.exit
        else:
            myPlace = None

        while myPlace is not None:
            if myPlace.ant is not None:

                if myPlace.ant not in self.buffed:
                    myPlace.ant.damage = myPlace.ant.damage * 2
                    self.buffed.append(myPlace.ant)

                if myPlace.ant.is_container and myPlace.ant.contained_ant is not None:
                    if myPlace.ant.contained_ant not in self.buffed:
                        myPlace.ant.contained_ant.damage = myPlace.ant.contained_ant.damage * 2
                        self.buffed.append(myPlace.ant.contained_ant)
...
def make_slow(action, bee): #bee param?
...
def make_scare(action, bee): #bee param?
