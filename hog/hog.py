...
def roll_dice(num_rolls, dice=six_sided): #clunky and wird
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    marker = False #marks true if there is ever a one called
    total = 0
...
def check_hog_wild(score0, score1):
    """returns true if the criteria for the special rule hog wild are met"""
    if score0 + score1 == 0: 
        return False
...
def swine_swap(score0, score1):
    string0 = str(score0)
    string1 = str(score1)
    a,s,d,f = string0[len(string0)-1], string0[len(string0)-2], string1[len(string1)-1], string1[len(string1)-2]
    diff0 = abs(int(a) - int(s))
    diff1 = abs(int(d) - int(f))
    if (diff0 == diff1):
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 <= goal and score1 <= goal:
        ...
        if player == 0:
            score = score0
            temp_strat = strategy0
            opponent = score1
            opp_strat = strategy1

        else:
            score = score1
            temp_strat = strategy1
            opponent = score0
            opp_strat = strategy0

        rolls = temp_strat(score, opponent)

        if check_hog_wild(score, opponent) == True:
            new_score = take_turn(rolls, opponent, four_sided)
        else:
            new_score = take_turn(rolls, opponent, dice)

        score = score + new_score
        
        if swine_swap(score, opponent) == True:
            place_holder = score
            score = opponent
            opponent = place_holder
